		...
		if (AutoCorrect_on && !message.startsWith("http")
			...
			if (message.length() > 1)
				for (int i = 1; i < message.length(); i++)
					if (!isNumberOrLetter(message.substring(i - 1, i))
							&& isNumberOrLetter(message.substring(i, i + 1))
	...
	public static int translateStringtoTimeInms(String written) {
		...
		for (String word : temp)
			if (!word.equalsIgnoreCase("and") && !word.equalsIgnoreCase("&"))
				words.add(replace(word.toLowerCase(), ",", "", true));
		while (words.size() > 0) {
